...
MAX_GAMES = 5000
MANUAL_RESTART =1  #4207The number of games to restart at (Change this by hand if you stop)
...
class Agent(BaseModel):
  ...
  def train(self):
    ...
    #Set a flag for saving the model
    save_flag = 1
    ...
    self.step = 0
    print("Wait for a bit while the Agent gets ready to learn...")
    print("This usually takes about 4 minutes or so...")
    while num_game <= MAX_GAMES:
      self.step += 1
      if self.step == self.learn_start:
        print("Learning commences... NOW!")
        num_game, self.update_count, ep_reward = MANUAL_RESTART, 0, 0.
        total_reward, self.total_loss, self.total_q = 0., 0., 0.
        ep_rewards, actions = [], []
      ...
      if terminal:   
        screen, reward, action, terminal = self.env.new_random_game()
        #Only start counting games from when learning starts:
        if self.step >= self.learn_start:
          num_game += 1
          print("Number of games completed: " + str(num_game) + " | Final reward: " + str(ep_reward))
          f = open('/Users/qq/Desktop/Results0.csv', 'a')
          try:
             writer = csv.writer(f)
             writer.writerow((str(num_game), str(ep_reward)))
          finally:
             f.close()
          save_flag = 1
      ...
      if self.step >= self.learn_start:
        ...
        ###Times to save the model:###
        #Every 500 games
        if num_game > 0 and num_game % 500 == 0 and save_flag == 1:
          save_flag = 0
          self.step_assign_op.eval({self.step_input: self.step + 1})
          self.save_model(num_game)
          self.save_weight_to_pkl()

        #And every 100,000 steps
        elif self.step % 99999 == 0:
          self.step_assign_op.eval({self.step_input: self.step + 1})
          self.save_model(num_game)
          self.save_weight_to_pkl()
        ###############################
        ...
        if self.step % self.test_step == self.test_step - 1:
          ...
          #Save the model when it's really good
    ...
    print("Highest reward: " + str(max(ep_rewards)))
    print("Average reward: " + str(sum(ep_rewards) / len(ep_rewards)))
  ...
  def build_dqn(self):
    ...
    self.load_model()
    if os.path.exists('weights/l2_w.pkl'):
  ...
  def play(self, n_step=10000, n_episode=100, test_ep=None, render=False):
    ...
    while num_games <= n_episode:
      ...
      if terminal:
        ...
        print("Game " + str(num_games) + " Completed. Total Reward: " + str(current_reward))
        current_reward = 0
        
    print("Best Reward: " + str(best_reward))
    print("Average Reward: " + str(total_reward / n_episode))
